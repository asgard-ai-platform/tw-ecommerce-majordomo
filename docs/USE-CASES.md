...
**情境：** 客戶要在 Shopline 1.0 開新店，需要同步把信用卡 / ATM / 超商代碼 / 超商取貨付款 / 電子發票串好。
...
**會用到的 MCPs：** `shopline`、`91app`（驗證上架後狀態）
...
**Prompt 範例：**
```
幫我做結帳頁的載具選擇 UI + 驗證邏輯：手機條碼（/開頭 8 碼）、自然人憑證
（16 碼大寫）、會員載具、捐贈碼（3-7 碼）。每種錯誤訊息給我寫好。
```
...
Plugin 直接服務「單次任務 / 單次規劃 / 單次除錯」場景。如果你要把它擺進一個多店、多品牌、有 agent team 治理需求的後台產品（類似 XXTECHEC 的 Asgard Fryer 規劃），Plugin 的 29 skills + 12 MCPs 會是底下的 building blocks，但上層還需要自己建：
